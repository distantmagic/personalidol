...
map objects. This can be used to produce any kind of game or poejct, from
...
2. [`unmarshalMap`](quakemaps-unmarshalMap.mdx) parses the map and creates
  ...
  [`EntitySketch`](quakemaps-EntitySketch.type) contains the entity properties
  that were provided in the map editor and an array of brushes (list of
  half-spaces that define the geometry).
