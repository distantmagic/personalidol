...
5. [`UIStateController`](personalidol-UIStateController) is the declarative
  ...
  [`UIStateController`](personalidol-UIStateController) to determine if the
...
7. [`DOMUIController`](dom-renderer-DOMUIController.mdx) can receive direct
  ...
  via the `UIState` and [`UIStateController`](personalidol-UIStateController) -
  primarily when the scene controller or view need to set specific properties
  to the DOM view. This disrupts the unidirectional data flow (which may be
  fine in some cases) and as a rule, `UIState` should be used for the sake of
  consistency.
