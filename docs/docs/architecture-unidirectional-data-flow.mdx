...
[`MouseState`](framework-MouseState.mdx),
[`TouchState`](framework-TouchState.mdx) are not active (they do not notify
...
- [`MouseObserver`](framework-MouseObserver.mdx) reads the dimensions array to
...
- [`TouchObserver`](framework-TouchObserver.mdx) does the same thing as
  [`MouseObserver`](framework-MouseObserver.mdx), but with touch input. It
  writes to the [`TouchState`](framework-TouchState.mdx).
...
- [`MapScene`](personalidol-MapScene.mdx) is a controller that reads from
  [`MouseState`](framework-MouseState.mdx)
  and [`TouchState`](framework-TouchState.mdx) to respond to user
  interactions.
